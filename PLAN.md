...
> 更新于 2026-03-17 | 当前版本 v0.5.0 | 当前阶段：安全加固全部完成，v0.5.0 已发布
...
| v0.5.0 | 2026-03-17 | 待发布 Release | 安全加固全部完成：脚本密码改环境变量、清理133个调试脚本、git 历史明文密码清除、GitHub PAT 轮换、grpc.NewClient 迁移 |

**当前 install.sh 默认版本**：`FREECDN_VERSION="v0.5.0"`  
**下载地址**：`https://github.com/hujiali30001/freecdn-admin/releases/download/v0.5.0/freecdn-v0.5.0-linux-{arch}.tar.gz`
...
- [x] **安全加固全部完成（v0.5.0）**：RED-01/02/03/07 + YEL-06/07/10 全部清零（2026-03-17）
  - scripts/e2e_full.py + build_linux.py 密码改读 `SSH_PASS` 环境变量，`AutoAddPolicy` → `WarningPolicy`
  - 删除 `scripts/rerun_install_fresh.py`（危险脚本），`.gitignore` 升级为白名单模式，133个调试脚本移出 Git 跟踪
  - `git filter-repo` 改写全部 2186 个 commit 历史，替换 3 种明文密码（`FreeCDN2026!` / `freecdn_pass_2026` / `freecdn_root_2026`），force push 至 GitHub
  - GitHub PAT 轮换（旧 token 撤销）
  - `grpc.Dial` / `grpc.DialContext` → `grpc.NewClient`（rpc_client.go + task_sync_api_nodes.go）
  - `v-html="page"` 审查通过（均为后端模板生成，无用户输入）
  - `security_init.go`（全局安全头兜底注册）加入版本管理
...
| RED-01 | SSH `AutoAddPolicy()` 改为 `WarningPolicy`（固定服务器） | `scripts/*.py` | ✅ |
...
| RED-04 | 轮换 MySQL 密码（`REDACTED_ROOT_PASS` / `REDACTED_DB_PASS`） | — | ⬜ |
...
| YEL-08 | 梳理 `context.TODO()` 调用，替换为请求上下文或带超时 context | ✅（13处全部改为 `context.Background()`，TeaGo action 层无请求 context 可用）|
| YEL-10 | 确认所有 `v-html="page"` 分页 HTML 均来自严格模板化的后端，不含用户输入 | ✅（审查通过，均为后端模板生成）|
