...
def mysql(sql):
    cmd = f"mysql -h 127.0.0.1 -u freecdn -p'FreeCDN2026!' freecdn -e \"{sql}\" 2>/dev/null"
    return run(cmd)

# 1. 检查当前 adminUIConfig
print("=== 1. 当前 adminUIConfig ===")
r = run("mysql -h 127.0.0.1 -u freecdn -p'FreeCDN2026!' freecdn -sNe \"SELECT CAST(value AS CHAR) FROM edgeSysSettings WHERE code='adminUIConfig';\" 2>/dev/null")
print(r or "(空/不存在)")
...
r = run(f"mysql -h 127.0.0.1 -u freecdn -p'FreeCDN2026!' freecdn -e \"{sql}\" 2>&1")
